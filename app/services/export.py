"""数据集导出为训练包（面向 Qwen-Image / ai-toolkit）。
...
# ai-toolkit 官方支持的 Qwen-Image 底模；也允许用户填写自定义模型名。
...
def resolve_hyperparams(opts: ExportOptions, image_count: int) -> dict[str, Any]:
    ...
    resolution = opts.resolution or [512, 768, 1024]
...
def build_aitoolkit_config(
    ...
    """生成 ai-toolkit 的 Qwen-Image LoRA 训练 YAML 配置。
    ...
    return f"""---
# ai-toolkit 训练配置（Qwen-Image LoRA），由 ImagesDataset 自动生成。
...
def build_export_zip(
    ...
    for im in selected:
        result = _prepare_image(getattr(im, "image_path", "") or "")
        if result is None:
            continue
        data, ext = result
    ...
    hp = resolve_hyperparams(opts, len(prepared))
    ...
    return buf.getvalue(), f"{slug}_qwen_image_lora.zip", len(prepared)
